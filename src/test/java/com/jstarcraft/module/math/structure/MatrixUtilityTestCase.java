...
import com.jstarcraft.module.math.algorithm.decomposition.SingularValueDecomposition;
...
public class MatrixUtilityTestCase {
	...
	public void test() throws Exception {
		...
		Future<?> task = context.doTask(() -> {
			...
			matrix.mapValues(MatrixMapper.ZERO, null, MathCalculator.PARALLEL);
			matrix.getRowVector(1).mapValues(VectorMapper.distributionOf(probability), null, MathCalculator.SERIAL);
			...
			matrix.mapValues(MatrixMapper.ZERO, null, MathCalculator.PARALLEL);
			matrix.getColumnVector(1).mapValues(VectorMapper.distributionOf(probability), null, MathCalculator.SERIAL);
			System.out.println(matrix);
			System.out.println(DenseMatrix.valueOf(4, 4).copyMatrix(matrix, true));
			System.out.println(DenseMatrix.valueOf(4, 4).dotProduct(matrix, true, matrix, false, MathCalculator.SERIAL));
	...
	public void testSparseMatrix() throws Exception {
		...
		Future<?> task = context.doTask(() -> {
			...
			sum = 0F;
			count = 0;
			matrix = SparseMatrix.copyOf(matrix, true, (row, column, value, message) -> {
				return value;
			});
			for (MatrixScalar term : matrix) {
				count++;
	...
	public void testDotProduct() throws Exception {
		EnvironmentContext context = Nd4j.getAffinityManager().getClass().getSimpleName().equals("CpuAffinityManager") ? EnvironmentContext.CPU : EnvironmentContext.GPU;
		Future<?> task = context.doTask(() -> {
			DenseMatrix matrix = DenseMatrix.valueOf(3, 4, MatrixMapper.distributionOf(probability));
	...
	public void testTransposeProduct() throws Exception {
		EnvironmentContext context = Nd4j.getAffinityManager().getClass().getSimpleName().equals("CpuAffinityManager") ? EnvironmentContext.CPU : EnvironmentContext.GPU;
		Future<?> task = context.doTask(() -> {
			DenseMatrix matrix = DenseMatrix.valueOf(4, 3, MatrixMapper.distributionOf(probability));
	...
	public void testPseudoinverse() throws Exception {
		...
		Future<?> task = context.doTask(() -> {
			...
			MatrixMapper mapper = (row, column, value, message) -> {
				if (value >= MathUtility.EPSILON) {
					Assert.fail();
				}
				return value;
			};
			...
			right.subtractMatrix(matrix, false).mapValues(mapper, null, MathCalculator.PARALLEL);
			...
			System.out.println(right);
			right.subtractMatrix(pseudoinverse, false).mapValues(mapper, null, MathCalculator.PARALLEL);
		});
		task.get();
	}
	...
	public void testSVD() throws Exception {
		...
		Future<?> task = context.doTask(() -> {
			...
			DenseMatrix oldMatrix = DenseMatrix.valueOf(rowSize, columnSize, (row, column, value, message) -> {
				return random.getAndIncrement();
			});
	...
	public void testSymmetryMatrix() throws Exception {
		...
		Future<?> task = context.doTask(() -> {
			...
			Assert.assertThat(left.dotProduct(symmetry, false, inverse, false, MathCalculator.SERIAL).mapValues(mapper, null, MathCalculator.PARALLEL), CoreMatchers.equalTo(right.dotProduct(inverse, false, symmetry, false, MathCalculator.SERIAL).mapValues(mapper, null, MathCalculator.PARALLEL)));

			inverse = MatrixUtility.inverse(cholesky, copy, inverse);
			Assert.assertThat(left.dotProduct(cholesky, false, inverse, false, MathCalculator.SERIAL).mapValues(mapper, null, MathCalculator.PARALLEL), CoreMatchers.equalTo(right.dotProduct(inverse, false, cholesky, false, MathCalculator.SERIAL).mapValues(mapper, null, MathCalculator.PARALLEL)));

			inverse = MatrixUtility.inverse(covariance, copy, inverse);
			Assert.assertThat(left.dotProduct(covariance, false, inverse, false, MathCalculator.SERIAL).mapValues(mapper, null, MathCalculator.PARALLEL), CoreMatchers.equalTo(right.dotProduct(inverse, false, covariance, false, MathCalculator.SERIAL).mapValues(mapper, null, MathCalculator.PARALLEL)));
		});
		task.get();
	}
