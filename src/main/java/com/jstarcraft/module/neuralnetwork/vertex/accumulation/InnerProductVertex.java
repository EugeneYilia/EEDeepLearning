...
public class InnerProductVertex extends AbstractVertex {
	...
	public void doBackward() {
		...
		for (int rowIndex = 0, rowSize = innerError.getRowSize(); rowIndex < rowSize; rowIndex++) {
			double error = innerError.getValue(rowIndex, 0);
			if (leftInputError != null) {
				...
				synchronized (leftInputError) {
					...
					leftInputError.getRowVector(rowIndex).mapValues((index, value, message) -> {
						return value += vector.getValue(index) * error;
			...
			if (rightInputError != null) {
				...
				synchronized (rightInputError) {
					...
					rightInputError.getRowVector(rowIndex).mapValues((index, value, message) -> {
						return value += vector.getValue(index) * error;
					}, null, MathCalculator.SERIAL);
				}
			}
