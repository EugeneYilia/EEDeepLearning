...
import com.jstarcraft.module.recommendation.recommender.collaborative.rating.BiasedMFRecommender;
...
public class TimeSVDRecommender extends BiasedMFRecommender {
	...
	private Table<Integer, Integer, Long> instantTabel;
	...
	public void prepare(Configuration configuration, SampleAccessor marker, InstanceAccessor model, DataSpace space) {
		...
		for (DataSample sample : marker) {
			instantTabel.put(sample.getDiscreteFeature(userDimension), sample.getDiscreteFeature(itemDimension), (long) sample.getContinuousFeature(instantDimension));
		}
		getMaxAndMinTimeStamp();
		numDays = days(maxTimestamp, minTimestamp) + 1;
		// TODO 考虑重构
		userBiases = DenseVector.valueOf(numberOfUsers, VectorMapper.RANDOM);
		itemBiases = DenseVector.valueOf(numberOfItems, VectorMapper.RANDOM);
		userBiasWeights = DenseVector.valueOf(numberOfUsers, VectorMapper.RANDOM);
		itemSectionBiases = DenseMatrix.valueOf(numberOfItems, numSections, MatrixMapper.RANDOM);
		itemImplicitFactors = DenseMatrix.valueOf(numberOfItems, numberOfFactors, MatrixMapper.RANDOM);
		userImplicitFactors = DenseMatrix.valueOf(numberOfUsers, numberOfFactors, MatrixMapper.RANDOM);
		userDayBiases = HashBasedTable.create();
		userDayFactors = HashBasedTable.create();
		userScales = DenseVector.valueOf(numberOfUsers, VectorMapper.RANDOM);
		userDayScales = DenseMatrix.valueOf(numberOfUsers, numDays, MatrixMapper.RANDOM);
		userExplicitFactors = DenseMatrix.valueOf(numberOfUsers, numberOfFactors, MatrixMapper.RANDOM);
		itemExplicitFactors = DenseMatrix.valueOf(numberOfItems, numberOfFactors, MatrixMapper.RANDOM);
		// global average date
		float mean;
		float sum = 0F;
		int count = 0;
		...
		for (int iterationStep = 1; iterationStep <= numberOfEpoches; iterationStep++) {
			...
			for (int userIndex = 0; userIndex < numberOfUsers; userIndex++) {
				...
				for (VectorScalar term : rateVector) {
					...
					// TODO 此处可以重构.
					long instant = instantTabel.get(userIndex, itemExplicitIndex);
					// day t
	...
	public float predict(int[] dicreteFeatures, float[] continuousFeatures) {
		...
		long instant = (long) continuousFeatures[instantDimension];
		int days = days(instant, minTimestamp);
		int section = section(days);
		float deviation = deviation(userIndex, days);
	...
	private void getMaxAndMinTimeStamp() {
		...
		for (Cell<Integer, Integer, Long> cell : instantTabel.cellSet()) {
			long timeStamp = cell.getValue();
