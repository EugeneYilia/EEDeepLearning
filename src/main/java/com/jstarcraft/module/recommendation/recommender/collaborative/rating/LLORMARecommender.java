...
import com.jstarcraft.module.math.algorithm.KernelSmoother;
...
public class LLORMARecommender extends MatrixFactorizationRecommender {
	...
	public void prepare(Configuration configuration, SampleAccessor marker, InstanceAccessor model, DataSpace space) {
		...
		globalUserFactors = DenseMatrix.valueOf(numberOfUsers, numberOfGlobalFactors, MatrixMapper.distributionOf(distribution));
		globalItemFactors = DenseMatrix.valueOf(numberOfItems, numberOfGlobalFactors, MatrixMapper.distributionOf(distribution));
	...
	private void practiceLocalModels(DefaultScalar scalar) {
		...
		while (completeModelCount < numberOfModels) {
			...
			if (runningThreadCount < numberOfThreads && modelCount < numberOfModels) {
				...
				int randomItemIndex = userVector.randomKey();
				...
				DenseMatrix localUserFactors = DenseMatrix.valueOf(numberOfUsers, numberOfLocalFactors, MatrixMapper.distributionOf(distribution));
				DenseMatrix localItemFactors = DenseMatrix.valueOf(numberOfItems, numberOfLocalFactors, MatrixMapper.distributionOf(distribution));
