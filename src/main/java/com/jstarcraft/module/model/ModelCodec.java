...
public enum ModelCodec {
	...
	private ModelData saveModel(Object model) {
		try {
			...
			String json = JsonUtility.object2String(java);
			...
			if (clazz.isArray()) {
				...
				if (specification == CodecSpecification.BOOLEAN || specification == CodecSpecification.NUMBER || specification == CodecSpecification.STRING || specification == CodecSpecification.TIME) {
	...
	private Object loadModel(ModelData content) {
		try {
			...
			Class<?> clazz = TypeUtility.getRawType(TypeUtility.convertType(java), null);
