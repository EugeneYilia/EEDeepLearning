...
public class CompositeVector implements MathVector, MathMonitor<VectorScalar> {
	...
	@Override
	public <T extends MathMessage> CompositeVector collectValues(VectorCollector<T> collector, T message, MathCalculator mode) {
		int size = components.length;
		for (int point = 0; point < size; point++) {
			MathVector vector = components[point];
			int split = splits[point];
			vector.collectValues((index, value, information) -> {
				collector.collect(index + split, value, information);
			}, message, mode);
		}
		return this;
	}
	...
	@Override
	public <T extends MathMessage> CompositeVector mapValues(VectorMapper<T> mapper, T message, MathCalculator mode) {
		int size = components.length;
		for (int point = 0; point < size; point++) {
			MathVector vector = components[point];
			int split = splits[point];
			vector.mapValues((index, value, information) -> {
				return mapper.map(index + split, value, information);
			}, message, mode);
		}
		return this;
	}
