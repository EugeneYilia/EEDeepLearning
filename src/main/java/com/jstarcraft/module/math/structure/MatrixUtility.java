...
import com.jstarcraft.module.math.structure.message.NonZeroMeanMessage;
...
public class MatrixUtility {
	...
	public static <T extends MathMatrix> T covariance(final MathMatrix matrix, MathVector outerMeans, MathVector innerMeans, T covariance) {
		...
		for (int outer = 0; outer < size; outer++) {
			...
			outerMeans.mapValues(VectorMapper.copyOf(matrix.getColumnVector(outer)), null, MathCalculator.SERIAL);
			NonZeroMeanMessage outerMeanMessage = new NonZeroMeanMessage();
			outerMeans.collectValues(VectorCollector.ACCUMULATOR, outerMeanMessage, MathCalculator.SERIAL);
			float outerMean = outerMeanMessage.getValue();
			outerMeans.mapValues((index, value, message) -> {
				return value - outerMean;
			...
			for (int inner = outer + 1; inner < size; inner++) {
				innerMeans.mapValues(VectorMapper.copyOf(matrix.getColumnVector(inner)), null, MathCalculator.SERIAL);
				NonZeroMeanMessage innerMeanMessage = new NonZeroMeanMessage();
				innerMeans.collectValues(VectorCollector.ACCUMULATOR, innerMeanMessage, MathCalculator.SERIAL);
				float innerMean = innerMeanMessage.getValue();
				innerMeans.mapValues((index, value, message) -> {
					return value - innerMean;
				}, null, MathCalculator.SERIAL);
	...
	public static <T extends MathMatrix> T inverse(final MathMatrix matrix, MathMatrix copy, T inverse) {
		...
		inverse.mapValues(MatrixMapper.ZERO, null, MathCalculator.PARALLEL);
	...
	public static <T extends MathMatrix> T wishart(final MathMatrix matrix, final ContinuousProbability normalDistribution, final ContinuousProbability[] gammaDistributions, MathVector randoms, MathMatrix cholesky, MathMatrix gaussian, MathMatrix gamma, MathMatrix transpose, T wishart) {
		...
		gaussian.mapValues(MatrixMapper.distributionOf(normalDistribution), null, MathCalculator.SERIAL);
		randoms.mapValues((index, value, message) -> {
			return gammaDistributions[index].sample().floatValue();
