 ...
 *
 */
// TODO 注意:最终目标是DenseMatrix基于DenseVector
// TODO data考虑支持integer和long类型
public class DenseMatrix implements MathMatrix {
	...
	@Override
	public <T extends MathMessage> DenseMatrix collectValues(MatrixCollector<T> collector, T message, MathCalculator mode) {
		switch (mode) {
		case SERIAL: {
			for (int rowIndex = 0; rowIndex < rowSize; rowIndex++) {
				for (int columnIndex = 0; columnIndex < columnSize; columnIndex++) {
					int cursor = rowIndex * columnSize + columnIndex;
					collector.collect(rowIndex, columnIndex, values[cursor], message);
				}
			}
			return this;
		}
			...
			if (this.getColumnSize() <= this.getRowSize()) {
				...
				for (int index = 0; index < size; index++) {
					...
					context.doStructureByAny(index, () -> {
						...
						for (int rowIndex = 0; rowIndex < rowSize; rowIndex++) {
							int cursor = rowIndex * columnSize + columnIndex;
							collector.collect(rowIndex, columnIndex, values[cursor], copy);
						}
						semaphore.release();
					});
				}
	...
	@Override
	public <T extends MathMessage> DenseMatrix mapValues(MatrixMapper<T> mapper, T message, MathCalculator mode) {
		switch (mode) {
		case SERIAL: {
			for (int rowIndex = 0; rowIndex < rowSize; rowIndex++) {
				for (int columnIndex = 0; columnIndex < columnSize; columnIndex++) {
					int cursor = rowIndex * columnSize + columnIndex;
					values[cursor] = mapper.map(rowIndex, columnIndex, values[cursor], message);
				}
			}
			return this;
		}
			...
			if (this.getColumnSize() <= this.getRowSize()) {
				...
				for (int index = 0; index < size; index++) {
					...
					context.doStructureByAny(index, () -> {
						...
						for (int rowIndex = 0; rowIndex < rowSize; rowIndex++) {
							int cursor = rowIndex * columnSize + columnIndex;
							values[cursor] = mapper.map(rowIndex, columnIndex, values[cursor], copy);
						}
						semaphore.release();
					});
				}
	...
	public static DenseMatrix copyOf(DenseMatrix matrix, MatrixMapper<?> mapper) {
		return DenseMatrix.copyOf(matrix, new float[matrix.rowSize * matrix.columnSize], mapper);
	}

	public static DenseMatrix copyOf(DenseMatrix matrix, float[] data, MatrixMapper<?> mapper) {
		...
		for (int row = 0; row < matrix.rowSize; row++) {
			for (int column = 0; column < matrix.columnSize; column++) {
				...
				instance.values[index] = mapper.map(row, column, matrix.values[index], null);
			}
		}
		return instance;
	}
	...
	public static DenseMatrix valueOf(int rowSize, int columnSize, MatrixMapper<?> mapper) {
		return DenseMatrix.valueOf(rowSize, columnSize, new float[rowSize * columnSize], mapper);
	}

	public static DenseMatrix valueOf(int rowSize, int columnSize, float[] data, MatrixMapper<?> mapper) {
		DenseMatrix instance = new DenseMatrix(rowSize, columnSize, data);
		for (int row = 0; row < rowSize; row++) {
			for (int column = 0; column < columnSize; column++) {
				int index = row * instance.columnSize + column;
				instance.values[index] = mapper.map(row, column, 0F, null);
			}
		}
		return instance;
	}
