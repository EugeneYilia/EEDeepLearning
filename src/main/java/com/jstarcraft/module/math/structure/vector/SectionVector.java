...
public class SectionVector implements MathVector {
	...
	@Override
	public <T extends MathMessage> SectionVector collectValues(VectorCollector<T> collector, T message, MathCalculator mode) {
		switch (mode) {
		case SERIAL: {
			for (int index = 0; index < elementSize; index++) {
				collector.collect(index, dataVector.getValue(index + from), message);
			}
			return this;
		}
		default: {
			...
			for (int index = 0; index < elementSize; index++) {
				...
				context.doStructureByAny(index, () -> {
					...
					collector.collect(elementIndex, dataVector.getValue(elementIndex + from), copy);
					semaphore.release();
				});
			}
			try {
	...
	@Override
	public <T extends MathMessage> SectionVector mapValues(VectorMapper<T> mapper, T message, MathCalculator mode) {
		switch (mode) {
		case SERIAL: {
			for (int index = 0; index < elementSize; index++) {
				dataVector.setValue(index + from, mapper.map(index, dataVector.getValue(index + from), message));
			}
			return this;
		}
		default: {
			...
			for (int index = 0; index < elementSize; index++) {
				...
				context.doStructureByAny(index, () -> {
					T copy = storage.detachMessage(message);
					dataVector.setValue(elementIndex + from, mapper.map(elementIndex, dataVector.getValue(elementIndex + from), copy));
					semaphore.release();
				});
			}
			try {
