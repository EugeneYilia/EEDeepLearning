...
public class RowCompositeMatrix extends CompositeMatrix {
	...
	@Override
	public <T extends MathMessage> RowCompositeMatrix collectValues(MatrixCollector<T> collector, T message, MathCalculator mode) {
		int size = components.length;
		for (int point = 0; point < size; point++) {
			MathMatrix matrix = components[point];
			int split = splits[point];
			matrix.collectValues((row, column, value, information) -> {
				collector.collect(row + split, column, value, information);
			}, message, mode);
		}
		return this;
	}
	...
	@Override
	public <T extends MathMessage> RowCompositeMatrix mapValues(MatrixMapper<T> mapper, T message, MathCalculator mode) {
		int size = components.length;
		for (int point = 0; point < size; point++) {
			MathMatrix matrix = components[point];
			int split = splits[point];
			matrix.mapValues((row, column, value, information) -> {
				return mapper.map(row + split, column, value, information);
			}, message, mode);
		}
		return this;
	}
