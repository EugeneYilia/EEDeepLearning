...
public class SymmetryVector implements MathVector {
	...
	@Override
	public <T extends MathMessage> SymmetryVector collectValues(VectorCollector<T> collector, T message, MathCalculator mode) {
		switch (mode) {
		case SERIAL: {
			for (int position = 0; position < dimension; position++) {
				int x, y;
				...
				int cursor = position;
				int index = point[x] + y;
				collector.collect(cursor, data[index], message);
			}
			return this;
		}
		default: {
			...
			for (int position = 0; position < dimension; position++) {
				...
				context.doStructureByAny(position, () -> {
					...
					collector.collect(cursor, data[index], copy);
					semaphore.release();
				});
			}
			try {
	...
	@Override
	public <T extends MathMessage> SymmetryVector mapValues(VectorMapper<T> mapper, T message, MathCalculator mode) {
		switch (mode) {
		case SERIAL: {
			for (int position = 0; position < dimension; position++) {
				int x, y;
				...
				int cursor = position;
				int index = point[x] + y;
				data[index] = mapper.map(cursor, data[index], message);
			}
			return this;
		}
		default: {
			...
			for (int position = 0; position < dimension; position++) {
				...
				context.doStructureByAny(position, () -> {
					T copy = storage.detachMessage(message);
					data[index] = mapper.map(cursor, data[index], copy);
					semaphore.release();
				});
			}
			try {
