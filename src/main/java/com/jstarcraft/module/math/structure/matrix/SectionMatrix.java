...
import com.jstarcraft.module.math.structure.vector.SectionVector;
...
public class SectionMatrix implements MathMatrix {
	...
	@Override
	public <T extends MathMessage> SectionMatrix collectValues(MatrixCollector<T> collector, T message, MathCalculator mode) {
		switch (mode) {
		case SERIAL: {
			for (int rowIndex = 0; rowIndex < rowSize; rowIndex++) {
				for (int columnIndex = 0; columnIndex < columnSize; columnIndex++) {
					collector.collect(rowIndex, columnIndex, matrix.getValue(rowIndex + top, columnIndex + left), message);
				}
			}
			return this;
		}
			...
			if (this.getColumnSize() <= this.getRowSize()) {
				...
				for (int index = 0; index < size; index++) {
					...
					context.doStructureByAny(index, () -> {
						...
						for (int rowIndex = 0; rowIndex < rowSize; rowIndex++) {
							collector.collect(rowIndex, columnIndex, matrix.getValue(rowIndex + top, columnIndex + left), copy);
						}
						semaphore.release();
					});
				}
	...
	@Override
	public <T extends MathMessage> SectionMatrix mapValues(MatrixMapper<T> mapper, T message, MathCalculator mode) {
		switch (mode) {
		case SERIAL: {
			for (int rowIndex = 0; rowIndex < rowSize; rowIndex++) {
				for (int columnIndex = 0; columnIndex < columnSize; columnIndex++) {
					matrix.setValue(rowIndex + top, columnIndex + left, mapper.map(rowIndex, columnIndex, matrix.getValue(rowIndex + top, columnIndex + left), message));
				}
			}
			return this;
		}
			...
			if (this.getColumnSize() <= this.getRowSize()) {
				...
				for (int index = 0; index < size; index++) {
					...
					context.doStructureByAny(index, () -> {
						T copy = storage.detachMessage(message);
						for (int rowIndex = 0; rowIndex < rowSize; rowIndex++) {
							matrix.setValue(rowIndex + top, columnIndex + left, mapper.map(rowIndex, columnIndex, matrix.getValue(rowIndex + top, columnIndex + left), copy));
						}
						semaphore.release();
					});
				}
