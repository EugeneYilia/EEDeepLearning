...
import com.jstarcraft.module.math.structure.vector.SymmetryVector;
...
public class SymmetryMatrix implements MathMatrix {
	...
	@Override
	public <T extends MathMessage> SymmetryMatrix collectValues(MatrixCollector<T> collector, T message, MathCalculator mode) {
		switch (mode) {
		case SERIAL: {
			int rowIndex = 0, columnIndex = 0, cursor = 0, size = data.length;
			while (cursor < size) {
	...
	@Override
	public <T extends MathMessage> SymmetryMatrix mapValues(MatrixMapper<T> mapper, T message, MathCalculator mode) {
		switch (mode) {
		case SERIAL: {
			int rowIndex = 0, columnIndex = 0, cursor = 0, size = data.length;
			while (cursor < size) {
				data[cursor] = mapper.map(rowIndex, columnIndex, data[cursor], message);
				if (columnIndex < rowIndex) {
					columnIndex++;
				} else {
					rowIndex++;
			...
			if (this.getColumnSize() <= this.getRowSize()) {
				...
				for (int index = 0; index < size; index++) {
					...
					context.doStructureByAny(index, () -> {
						...
						for (int rowIndex = dimension - 1; rowIndex >= columnIndex; rowIndex--) {
							int cursor = point[rowIndex] + columnIndex;
							data[cursor] = mapper.map(rowIndex, columnIndex, data[cursor], copy);
						}
						semaphore.release();
					});
				}
