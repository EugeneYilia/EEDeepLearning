package com.jstarcraft.module.math.algorithm.decomposition;
 ...
 * <p>
 * For an m-by-n matrix A with {@code m >= n}, the singular value decomposition
 * is an m-by-n orthogonal matrix U, an n-by-n diagonal matrix S, and an n-by-n
 * orthogonal matrix V so that A = U*S*V'. Note that this implementation
 * requires {@code m>=n}. Otherwise, you'd better use the transpose of a matrix.
 * <p>
...
public class SingularValueDecomposition {
	...
	public SingularValueDecomposition(MathMatrix matrix) {
		...
		for (int index = 0; index < Math.max(bidiagonalColumn, bidiagonalRow); index++) {
			if (index < bidiagonalColumn) {
				...
				for (int row = index; row < rowSize; row++) {
					singularValues[index] = MathUtility.hypot(singularValues[index], data[row][index]);
				}
			...
			if (index < bidiagonalRow) {
				...
				for (int column = index + 1; column < columnSize; column++) {
					eigenValues[index] = MathUtility.hypot(eigenValues[index], eigenValues[column]);
				}
		...
		while (outerPosition > 0) {
			...
			case 1: {
				...
				for (int row = outerPosition - 2; row >= outerIndex; row--) {
					float transformation = MathUtility.hypot(singularValues[row], eigenValue);
			...
			case 2: {
				...
				for (int column = outerIndex; column < outerPosition; column++) {
					float transformation = MathUtility.hypot(singularValues[column], eigenValue);
			...
			case 3: {
				...
				for (int column = outerIndex; column < outerPosition - 1; column++) {
					float transformation = MathUtility.hypot(singularValue, eigenValue);
					...
					transformation = MathUtility.hypot(singularValue, eigenValue);
		...
		singularVector = DenseVector.valueOf(Math.min(rowSize + 1, columnSize), (index, value, message) -> {
			return singularValues[index];
		});
		uMatrix = DenseMatrix.valueOf(rowSize, columnSize, (row, column, value, message) -> {
			return uValues[row][column];
		});
		vMatrix = DenseMatrix.valueOf(columnSize, columnSize, (row, column, value, message) -> {
			return vValues[row][column];
		});
	}
